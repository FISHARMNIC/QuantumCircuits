...
import { abs, square, i, minus_i, divide, add, subtract, sqrt, multiply, to_complex, type ComplexVector } from "./Math.js";

export let inputProbabilities: ComplexVector = (eval(probsInput.value) as ComplexVector).map(x => to_complex(x));

export const settingsSetup = (): void => {
    ...
    const enterFn = () => {
        try{
        const probs = (eval(probsInput.value) as number[]).map(x => to_complex(x));
